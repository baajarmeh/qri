...
type SaveParams struct {
	...
	// TODO(dustmop): Perhaps move this UseDscache (and all others) to scope?
	UseDscache bool
}
...
func (datasetImpl) Get(scope scope, p *GetParams) (*GetResult, error) {
	...
	ref, _, err := scope.ParseAndResolveRefWithWorkingDir(scope.Context(), p.Refstr, "")
	if err != nil {
		return nil, err
	}
	// TODO(dustmop): source
	ds, err = scope.Loader().LoadResolved(scope.Context(), ref)
	if err != nil {
		return nil, err
	}
...
func (datasetImpl) Stats(scope scope, p *StatsParams) (*dataset.Stats, error) {
	...
	if ds == nil {
		var err error
		ds, err = scope.Loader().LoadDataset(scope.Context(), p.Refstr)
		if err != nil {
			return nil, err
		}
	}
